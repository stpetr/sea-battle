...
gameSchema.methods.makeShot = async function (playerId, row, col) {
    ...
    const hasSameCoordsShot = this.getPlayerShots(playerId).some(({ row: shotRow, col: shotCol }) => shotRow === row && shotCol === col)
...
gameSchema.methods.isPlayerWon = function (playerId) {
    ...
    const totalShipCellsNum = this.getPlayerShips(playerId).reduce((accumulator, { width, length} ) => accumulator + width * length, 0)
